...
class NewsFilterSettings(BaseModel):
    ...
    @classmethod
    def api_key_required_for_provider(cls, v, info: ValidationInfo):
        provider = info.data.get('api_provider')
    ...
    @field_validator('manual_news_file_path')
    @classmethod
    def manual_file_path_required(cls, v, info: ValidationInfo):
        if info.data.get('api_provider') == "Manual" and not v:
